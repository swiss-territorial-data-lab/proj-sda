...
if __name__ == "__main__":
    ...
    completed_detections_gdf.drop(columns=['wkb_geom'], inplace=True)
    ...
    filtered_groupped_dets_gdf = groupped_detections_gdf[groupped_detections_gdf.merged_score>THRESHOLD]
    removed_dets_gdf = groupped_detections_gdf[groupped_detections_gdf.merged_score<=THRESHOLD]
    ...
    intersecting_dets_gdf = pd.concat(
        [
            overlap_detections_gdf[['merged_id_left', 'IoU_left']].groupby(['merged_id_left'], as_index=False).max().rename(
                columns={'merged_id_left':'merged_id', 'IoU_left':'second_IoU'}
            ), 
            overlap_detections_gdf[['merged_id_right', 'IoU_right']].groupby(['merged_id_right'], as_index=False).max().rename(
                columns={'merged_id_right':'merged_id', 'IoU_right':'second_IoU'}
            )
        ],
        ignore_index=True
    )
    intersecting_dets_gdf.sort_values('second_IoU', inplace=True, ascending=False)
    intersecting_dets_gdf.drop_duplicates(subset='merged_id', inplace=True)

    # Remove dets under another det
    filtered_groupped_dets_gdf = pd.merge(filtered_groupped_dets_gdf, intersecting_dets_gdf, how='left', on='merged_id')
    ...
    final_groupped_dets_gdf.to_file(filepath, crs='EPSG:2056', driver='GPKG', index=False)
    written_files.append(filepath)

    filepath = os.path.join(OUTPUT_DIR, 'removed_detections.gpkg')
    removed_dets_gdf.to_file(filepath, crs='EPSG:2056', driver='GPKG', index=False)
    written_files.append(filepath)
